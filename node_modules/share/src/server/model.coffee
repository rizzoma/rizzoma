...
module.exports = Model = (db, options) ->
    ...
    if docs[docName]
      ...
      if callbacks
        ...
        db.getSnapshot docName, (error, data, dbMeta) ->
          ...
          applyFutureOps docName, data, (error, ops) ->
            return callback error if error
            add docName, error, data, committedVersion, ops, dbMeta

  applyFutureOps = (docName, doc, callback) ->
    getOpsInternal docName, doc.v, null, (error, ops) ->
      return callback error if error
      if ops.length > 0
      ...
      if doc == docs[docName] and
        ...
        doc.reapTimer = reapTimer = setTimeout ->
            tryWriteSnapshot docName, ->
              # Somehow node 0.4.9 do not clear timeouts, so we have to check
              # that document wasn't deleted
              return if not docs[docName]?
              ...
              if doc.eventEmitter.listeners('op').length > 0
                ...
                return
              if docs[docName].reapTimer is reapTimer and doc.opQueue.busy is false
                delete docs[docName]
          , options.reapTime
  ...
  tryWriteSnapshot = (docName, callback) ->
    ...
    writeSnapshot docName, data, doc.dbMeta, (error, dbMeta) ->
      ...
      doc.dbMeta = dbMeta
      doc.meta._rev = dbMeta if not error
      callback? error
  ...
  @add = (docName, doc, version, ops) ->
    add(docName, null, doc, version, ops, null)

  @hasDocInCache = (docName) ->
    return !!@getDocFromCache(docName) 

  @getDocFromCache = (docName) ->
    return docs[docName]

  @compactCache = (docName) ->
    getDoc = (docName) ->
      doc = docs[docName]
      return if not doc or not doc.snapshot or not doc.meta
      rev = doc.meta._rev
      return if not rev
      return doc
    doc = getDoc(docName)
    return if not doc
    rev = doc.meta._rev
    onTimeout = (docName, rev) ->
      doc = getDoc(docName)
      return if (not doc) or (doc.meta._rev != rev) or (doc.opQueue.queue.length) or (doc.opQueue.busy)
      return if doc.neededToPostop or doc.neededToSubscribe
      delete doc.snapshot
      delete doc.meta
    setTimeout(onTimeout, 1000, docName, rev)
