...
class MessagingController
    ...
    sendToUser: (url, fromUser, fromResource, toUserId, toResource, message, ttl, callback) ->
        return callback(new Error('User is anonymous'), null) if fromUser.isAnonymous()
        tasks = [
            async.apply(WaveController.getWaveByUrl, url, fromUser)
            (wave, callback) =>
                return callback(new Error('toUser not found'), null) unless wave.hasParticipantById(toUserId)
                @_queueMessage(fromUser, fromResource, toUserId, toResource, message, ttl, callback)
        ]
        async.waterfall(tasks, callback)

    _queueMessage: (fromUser, fromResource, toUserId, toResource, message, ttl, callback) ->
        fromUserId = fromUser.id
        #console.log({fromUserId, fromResource, toUserId, toResource, message, ttl, callback, subscriptions: @_subscriptions})
        messageData =
            fromUserId: fromUserId
            fromResource: fromResource
            message: message
        # TODO: сейчас никакой очереди нет, сразу делается отправка, ttl тоже не учитывается
        # TODO: персистентное хранилище для сообщений
        if @_subscriptions[toUserId]?
            if toResource?
                if @_subscriptions[toUserId][toResource]?
                    @_subscriptions[toUserId][toResource](null, messageData)
                    status = "sent"
                else
                    status = "no such resource"
            else
                for r, cb of @_subscriptions[toUserId]
                    cb(null, messageData) 
                status = "sent (all resources)"
        else
            status = "user offline"
        
        # TODO: сообщать ли, что пользователя нет онлайн (?)
        callback(null, {"status": status})

    subscribeUser: (user, resource, callback) ->
        # TODO: очищать подписки после дисконнекта пользователя (или хотя бы по ttl)
        # TODO: после реализации очереди при подключении отправлять накопившиеся сообщения (возможно, нужен
        #       параметр from - до какого момента получены сообщения)
        return callback(new Error('User is anonymous'), null) if user.isAnonymous()
        userId = user.id
        @_subscriptions[userId] = @_subscriptions[userId] or {}
        @_subscriptions[userId][resource] = callback
        callback(null)
...
module.exports.MessagingController = new MessagingController()
